...
@implementation ApplifierSDURLCache
...
#pragma mark ApplifierSDURLCache (tools)
...
+ (NSDate *)expirationDateFromHeaders:(NSDictionary *)headers withStatusCode:(NSInteger)status
{
    ...
    if (date)
    {
        now = [ApplifierSDURLCache dateFromHttpDateString:date];
    }
    else
    {
    ...
    if (expires)
    {
        ...
        NSDate *expirationDate = [ApplifierSDURLCache dateFromHttpDateString:expires];
        if (expirationDate)
        {
    ...
    if (lastModified)
    {
        ...
        NSDate *lastModifiedDate = [ApplifierSDURLCache dateFromHttpDateString:lastModified];
        if (lastModifiedDate)
        {
...
#pragma mark ApplifierSDURLCache (private)
...
- (void)storeToDisk:(NSDictionary *)context
{
    ...
    NSString *cacheKey = [ApplifierSDURLCache cacheKeyForURL:request.URL];
...
#pragma mark ApplifierSDURLCache

+ (NSString *)defaultCachePath
{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    return [[paths objectAtIndex:0] stringByAppendingPathComponent:@"ApplifierSDURLCache"];
}
...
{
    request = [ApplifierSDURLCache canonicalRequestForRequest:request];
        ...
        && cachedResponse.data.length < self.diskCapacity)
    {
        ...
        {
            NSDate *expirationDate = [ApplifierSDURLCache expirationDateFromHeaders:headers
...
- (void)removeCachedResponseForRequest:(NSURLRequest *)request
{
    ...
    [self removeCachedResponseForCachedKeys:[NSArray arrayWithObject:[ApplifierSDURLCache cacheKeyForURL:request.URL]]];
    [self saveCacheInfo];
}
...
- (BOOL)isCached:(NSURL *)url
{
    ...
    NSString *cacheKey = [ApplifierSDURLCache cacheKeyForURL:url];
